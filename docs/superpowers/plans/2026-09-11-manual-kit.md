...
The first/last-chapter cases are **two tests**, not one. A single test calling `setup` twice
leaves both trees mounted in the same document, and a count-based assertion then passes on the
sum across them rather than on the chapter under test — which is a green test measuring the wrong
thing. Testing Library cleans up between tests, so splitting is the honest fix; never reach for
`cleanup()` mid-test.
