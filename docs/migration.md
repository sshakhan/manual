...
| — (не было) | `--manual-breakpoint-narrow`, `--manual-breakpoint-wide` | Новые токены, документируют точки перелома раскладки; сама раскладка на момент переноса переключается по захардкоженным `900px`/`1200px` в CSS пакета, а не по этим токенам — переопределение не изменит точку перелома. См. `docs/tokens.md` |
