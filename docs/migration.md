...
Перенос `:root`-токенов не переносит сброс `html, body { margin: 0; padding:
0 }`, который был в старом `src/styles.css`, — токены и сброс страницы это
разные вещи. Пакет намеренно не задаёт `body { margin: 0 }` сам (`@layer
base` ограничен `.manual`, оболочка не может переопределять стили хоста), так
что этот сброс нужно перенести в `theme.css` вручную — иначе вокруг
`min-block-size: 100vh` оболочки останется зазор в 8px от дефолтного отступа
браузера.
...
Пробел в переводе (глава есть в базовой локали, но не в остальных) по
умолчанию — предупреждение, а не ошибка сборки. Оба руководства сегодня
переведены полностью, и раньше сборка падала на любой пропавшей главе;
переезд на пакет без изменений тихо превратил бы этот гейт в необязательный.
Чтобы сохранить сегодняшнее поведение, используйте строгий режим:

```json
{
  "scripts": {
    "validate": "manual-kit validate --strict"
  }
}
```
